...
export default function Rootwork({ pose }) {
  ...
  const handleConfirmBatchImport = async (readyProducts, isComplete) => {
    ...
    setImportStatus(`Etching ${readyProducts.length} ready items into the Codex...`);
          ...
          <div className="modal-content card" style={{maxWidth: '500px'}}>
            ...
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start'}}>
              <div>
                ...
                <div className="mt mb-4" style={{color: 'var(--plum)'}}>Etch a new artifact to your Reliquary or vessel to the Apothecary.</div>
              </div>
              {modalState !== 'manual' && (
                <button className="btn sm" style={{padding: '0.4rem 1rem', whiteSpace: 'nowrap', flexShrink: 0}} onClick={() => setModalState('manual')} title="Manual Etch">
                  Etch by Hand
                </button>
              )}
            </div>
